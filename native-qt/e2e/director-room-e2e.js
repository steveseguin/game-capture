...
async function runRemoteControlContract({ page, uuid, config, publisher, report, check }) {
  ...
  const deviceProbeStart = await getDirectorProbeMessageCount(page);
  await contractCheck(enabled ? 'device-change-is-unsupported' : 'device-change-is-unauthorized', async () => {
    const request = await sendDirectorRequest(page, uuid, {
      changeCamera: 'game-capture-e2e-nonexistent-device'
    });
    const rejection = request.ok
      ? await waitForProbeMessage(
        page,
        `(entry) => {
          const msg = entry && entry.message;
          return msg && msg.rejected === 'changeCamera' && msg.reason === '${enabled ? 'unsupported' : 'unauthorized'}' &&
            typeof msg.message === 'string' && msg.message.length > 0;
        }`,
        5000,
        deviceProbeStart
      )
      : { ok: false, stage: 'send-device-change', state: request };
    const alerts = await settleUnsupportedAlerts(page, 6000);
    return { ok: request.ok && rejection.ok && alerts.ok, state: { request, rejection, alerts } };
  });
  ...
  if (bitrateAction.ok) {
    if (enabled) {
      ...
      await contractCheck('bitrate-rejection-is-unauthorized', async () => {
        const rejection = await waitForProbeMessage(
          page,
          `(entry) => {
            const msg = entry && entry.message;
            return msg && msg.rejected === 'targetBitrate' &&
              msg.reason === 'unauthorized' && typeof msg.message === 'string' && msg.message.length > 0;
          }`,
          5000
        );
        // VDO.Ninja can queue separate legacy alerts for the shared and
        // per-peer bitrate requests generated by one visible control.
        const alerts = await settleUnsupportedAlerts(page, 6000);
        return { ok: rejection.ok && alerts.ok, state: { rejection, alerts } };
      });
  ...
  if (openedSettings.ok) {
    ...
    const resolutionAction = await contractCheck('resolution-visible-control-action', async () => {
      const action = await changeDirectorResolutionThroughUi(
        page,
        uuid,
        targetResolution.width,
        targetResolution.height
      );
      if (enabled || !action.ok) return action;
      const alerts = await settleUnsupportedAlerts(page, 6000);
      return { ok: action.ok && alerts.ok, state: { action, alerts } };
    });
    if (resolutionAction.ok) {
      if (!enabled) {
        await contractCheck('resolution-rejection-is-unauthorized', async () => {
          const rejection = await waitForProbeMessage(
            page,
            `(entry) => {
              const msg = entry && entry.message;
              return msg && (msg.rejected === 'requestVideoHack' || msg.rejected === 'requestResolution') &&
                msg.reason === 'unauthorized' && typeof msg.message === 'string' && msg.message.length > 0;
            }`,
            5000
          );
  ...
  if (hangupAction.ok) {
    if (enabled) {
      ...
      await contractCheck('hangup-is-rejected', async () => {
        const rejection = await waitForProbeMessage(
          page,
          `(entry) => {
            const msg = entry && entry.message;
            return msg && msg.rejected === 'hangup' && msg.reason === 'unauthorized' &&
              typeof msg.message === 'string' && msg.message.length > 0;
          }`,
          5000
        );
...
async function run() {
  ...
  try {
    ...
    await check('director-quality-high-is-explicitly-unsupported', () => waitForProbeMessage(
      ...
      `(entry) => {
        ...
        return msg && (msg.rejected === 'bitrate' || msg.rejected === 'optimizedBitrate') &&
          msg.reason === 'unsupported' && typeof msg.message === 'string' && msg.message.length > 0;
      }`,
      Math.max(10000, Math.floor(config.timeoutMs / 3)),
    ...
    await check('vdo-preview-video-rate-is-explicitly-unsupported', () => waitForProbeMessage(
      ...
      `(entry) => {
        const msg = entry && entry.message;
        return msg && msg.rejected === 'bitrate' && msg.reason === 'unsupported' &&
          typeof msg.message === 'string' && msg.message.length > 0;
      }`,
      Math.max(10000, Math.floor(config.timeoutMs / 3)),
