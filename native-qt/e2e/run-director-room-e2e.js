...
const harnessPath = path.join(__dirname, 'director-room-e2e.js');
...
function resolveSpoutSender(buildDir) {
  let candidates = [];
  if (buildDir) {
    candidates = findFilesNamed(path.resolve(nativeRoot, buildDir), 'spout_test_sender.exe', 5);
  } else {
  ...
  if (candidates.length !== 1) {
    const detail = candidates.length > 0 ? ` Candidates: ${candidates.join(', ')}` : '';
    throw new Error(
...
function resolveCurrentPackage() {
  ...
  const packageDir = path.join(nativeRoot, 'dist', `game-capture-${versionMatch[1]}-win64`);
  ...
  for (const requiredPath of [publisherPath, artifactManifestPath]) {
    if (!fs.existsSync(requiredPath) || !fs.statSync(requiredPath).isFile()) {
      throw new Error(
        `Current packaged artifact is missing: ${requiredPath}. Run qa/build-release.ps1 first.`
      );
    }
  }
  let manifest;
...
function prepareDirectorArgs(sourceArgs, overrides = {}) {
  const dependencies = {
  ...
  if (explicitArtifactOptions.size === 0) {
    ...
    if (explicitArtifactOptions.size !== artifactOptions.size) {
  ...
  const firefoxIdentityOptions = new Set(['--firefox-path', '--expected-firefox-sha256']);
...
function main() {
  ...
  const resolveOnly = sourceArgs.includes('--resolve-only');
  const forwardedArgs = prepareDirectorArgs(
    sourceArgs.filter((arg) => arg !== '--resolve-only')
  );
  if (resolveOnly) {
    console.log(JSON.stringify(forwardedArgs, null, 2));
    return;
  }

  console.log('[CONTROL-CENTER-E2E] Exact packaged artifact identities resolved.');
  ...
  child.on('error', (error) => {
    console.error(`[CONTROL-CENTER-E2E] Failed to start harness: ${error.message}`);
    process.exitCode = 1;
  });
  child.on('exit', (code, signal) => {
    if (signal) {
      console.error(`[CONTROL-CENTER-E2E] Harness terminated by signal ${signal}`);
      process.exitCode = 1;
      return;
    }
...
module.exports = { prepareDirectorArgs };

if (require.main === module) {
  try {
    main();
  } catch (error) {
    console.error(`[CONTROL-CENTER-E2E] ${error.message}`);
    process.exitCode = 1;
  }
}
