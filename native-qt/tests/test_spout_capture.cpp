...
class SenderProcess {
  public:
    explicit SenderProcess(std::string name)
        : name_(std::move(name)) {}
...
void TestSpoutCapture::testReceivesBgraAlphaFrames() {
    ...
    for (size_t i = 3; i < frame.data.size(); i += 4) {
        hasTransparent = hasTransparent || frame.data[i] == 0;
        hasSemiTransparent = hasSemiTransparent || frame.data[i] == 128;
        hasOpaque = hasOpaque || frame.data[i] == 255;
    }

    QVERIFY(hasTransparent);
    QVERIFY(hasSemiTransparent);
    QVERIFY(hasOpaque);
