...
void TestWebRtcClient::testReservedAlphaIsInactiveUntilCapability() {
    ...
    const auto activation = client.ensureMediaTracks(true, true, true);
    QVERIFY(activation.changed);
    QVERIFY(activation.alphaDirectionChanged);
    const auto repeatedActivation = client.ensureMediaTracks(true, true, true);
    QVERIFY(!repeatedActivation.changed);
    QVERIFY(!repeatedActivation.alphaDirectionChanged);
    ...
    const auto deactivation = client.ensureMediaTracks(true, true, false);
    QVERIFY(deactivation.changed);
    QVERIFY(deactivation.alphaDirectionChanged);
